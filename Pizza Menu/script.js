const Pizza = [
  ...
  const pizzaBody = document.getElementById("pizzaBody");
  ...
  Pizza.forEach(pizza => {
    ...
    description.textContent = pizza.dsc;
  ...
  pizzaBody.appendChild(menuContainer);
  ...
  function updateCart(cart) {
    ...
    for (let i = 0; i < cart.length; i++) {
      ...
      priceTag.textContent = `Price: $${item.price}.00`;
  
      menuItem.append(pizzaName, priceTag, removeButton);
      cartList.appendChild(menuItem);
    }
